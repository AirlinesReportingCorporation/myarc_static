  ...
  */
	(function (a) {
		...
		if (jQ.support.cors || !jQ.ajaxTransport || !window.XDomainRequest) {
			return;
		}var n = /^https?:\/\//i;var o = /^get|post$/i;var p = new RegExp('^' + location.protocol, 'i');jQ.ajaxTransport('* text html xml json', function (j, k, l) {
					...
					var h = '';var i = (k.dataType || '').toLowerCase();m = new XDomainRequest();if (/^\d+$/.test(k.timeout)) {
						...
						var a = 'Content-Length: ' + m.responseText.length + '\r\nContent-Type: ' + m.contentType;var b = { code: 200, message: 'success' };var c = { text: m.responseText };try {
							if (i === 'html' || /text\/html/i.test(m.contentType)) {
								...
								try {
									c.json = jQ.parseJSON(m.responseText);
								} catch (e) {
						...
						h = jQ.type(k.data) === 'string' ? k.data : jQ.param(k.data);
	...
	!function (t, e) {
		...
		function t(t) {
			...
			var e = t._subscribers,
				...
				for (var r = void 0, o = void 0, i = t._result, s = 0; s < e.length; s += 3) r = e[s], o = e[s + n], r ? P(n, r, o, i) : o(i);t._subscribers.length = 0;
			}
		}function M(t, e) {
			try {
				return t(e);
	...
	var section = jQ(".wlp-bighorn-layout tr td").eq(1);
	...
	function setSegment(data) {
			...
			jQ.ajax({
				...
				for (var i = 0; i < data.length; i++) {
					if (data[i].entities.media != undefined) {
						...
						if (imageWidth == 1250 && imageHeight == 650 || imageWidth == 1250 && imageHeight == 625 || imageWidth == 650 && imageHeight == 325 || imageWidth == 600 && imageHeight == 300 || imageHeight == 417 && imageWidth == 833) {
		...
		switch (segment) {
			case "Internal":
				jQ(".dashboardPortlet").html(data["internal"].body);
				jQ("#Accordion2Content").html(data["internalResources"].body);
				break;

			case "Carrier":
				jQ(".dashboardPortlet").html(data["carrier"].body);
				jQ("#Accordion2Content").html(data["carrierResources"].body);
				break;

			case "Agency":
				jQ(".dashboardPortlet").html(data["agency"].body);
				jQ("#Accordion2Content").html(data["agencyResources"].body);  
				break;
			...
			case "LEO":
				jQ(".dashboardPortlet").html(data["LEO"].body);
				jQ("#Accordion2Content").html(data["LEOResources"].body);
				break;

			case "Tour Operator":
				jQ(".dashboardPortlet").html(data["touroperator"].body);
				jQ("#Accordion2Content").html(data["toResources"].body);
				break;

			case "3rd Party":
				jQ(".dashboardPortlet").html(data["3rdparty"].body);
				jQ("#Accordion2Content").html(data["3rdpartyResources"].body);
				break;

			case "Airport":
				jQ(".dashboardPortlet").html(data["airport"].body);
				jQ("#Accordion2Content").html(data["airportResources"].body);
				break;

			case "CTD":
				jQ(".dashboardPortlet").html(data["ctd"].body);
				jQ("#Accordion2Content").html(data["ctdResources"].body);
				break;

			case "System Provider":
				jQ(".dashboardPortlet").html(data["systemprovider"].body);
				jQ("#Accordion2Content").html(data["spResources"].body); 
				break;
	...
	// html DOM edits
	jQ("#ARC_Home").parent().parent().addClass("dashboardBG");
	jQ('img[alt="Pending Requests"]').parent().parent().parent().addClass("pendingRequests");
	jQ("#leftCol #myARCleftNavTop div").eq(0).addClass("accountInfo");
	jQ('#searchInput:after');

	/* if notifications */
	if (jQ("#alertBody1").length !== 0 || jQ(".alertframeLong").length !== 0) {

		var alertHtml = "";
		jQ(".alertframeLong").parent().addClass("alertParent");

		if (jQ("#alertPswd").attr("style") == "display:none;" || jQ("#alertPswd").attr("style") == "display: none;") {
			console.log("hidden alert");
		} else if (jQ(".alertframeLong").length !== 0 && jQ(".alertframeLong") !== null) {
			var framelonghtml = jQ(".alertframeLong").html().trim();
			...
			if (alertHtml != null && typeof alertHtml !== "undefined") {
				if (alertHtml.indexOf("check.png") !== -1) {
					jQ(".alertBar").css("background", "#b2def4");
				}
			}
		} else {
			jQ(".wlp-bighorn-header").after("<div class='alertBar'><div class='alertBar-inner'></div></div>");
			alertHtml = jQ("#alertBody1").html();
		}

		jQ(".alertBar-inner").html(alertHtml);
	}

	jQ(".wlp-bighorn-layout .wlp-bighorn-layout-cell").eq(1).addClass("supplementalSection");

	jQ("#nav2").html(jQ("#nav2").html().replace(/|/g, ""));

	jQ("#arcLogo").after("<div class='productName'><h2>My ARC</h2></div>");

	jQ("#pageFrameHdr").prepend("<div class='mobile-icon'><img src='https://www2.arccorp.com/globalassets/icons/hamburger-icon.jpg'></div>");

	jQ(".mobile-icon").click(function () {
		jQ("#searchNav").toggle();
	});
	...
	jQ("input").each(function () {
		...
		if (name == "user" || id == "usrName" || id == "userName") {
			jQ(this).attr("placeholder", "Username");
		} else if (name == "password" || id == "usrPswd") {
			jQ(this).attr("placeholder", "Password");
	...
	axios.get(marketingContentUrl + Math.floor(Math.random() * 100000000000)).then(function (response) {
